  ...
  Future<void> _deleteReadNotifications() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Bạn có chắc chắn muốn xóa tất cả thông báo đã đọc?'),
        actions: [
          TextButton(
  ...
  Widget _buildNotificationCard(NotificationModel item) {
    ...
    return Dismissible(
      ...
      onDismissed: (direction) async {
        try {
          await NotificationService().deleteNotification(item.maThongBao);
          _future = NotificationService().getNotifications();
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              ...
              const PopupMenuItem(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Xóa thông báo đã đọc', style: TextStyle(color: Colors.red)),
                  ],
                ),
              ),
            ],
